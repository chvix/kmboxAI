...
int main(int argc, char **argv){   
    ...
    {      
        ...
        if(kmAI_monitor_keyboard(KEY_B)){
            ...
            printf("键盘B键松开\n");
            kmAI_keydown(KEY_B);   printf("软件发送B键按下消息\n");//软件发送一次B键按下
            kmAI_keyup(KEY_B);     printf("软件发送B键松开消息\n");//软件发送一次B键松开
    ...
    printf("解除A/B按键屏蔽。C依旧屏蔽中  按C键退出程序.\n");
    while(kmAI_monitor_keyboard(KEY_C)==0) Delay100ms;//再次等待C键按下

    kmAI_unmask_all();//取消所有屏蔽
    printf("所有按键恢复正常\n");
   // kmAI_Run();//开始运行 ,此函数不会退出 ---如果想要继续操作电脑。解除这个的屏蔽。不然程序都退出了。肯定键鼠消息无法发送到OTG口
    return 0;
}
